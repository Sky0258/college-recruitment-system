...
import { reqAddEdu, reqAddPos, reqAdminAward, reqAdminEdu, reqAdminProject, reqChangeUser, reqDeletePos, reqShowApplyPos, reqShowPos, reqShowUsers, reqUpdatePos,reqShowCompanyPos, reqShowCompanyApplyPos} from "../api";
const state = {
    allPos:{},
    companyPos:{},
    allUsers:{},
...
const mutations = {
    ...
    COMPANYAPPLYPOS(state,companyApplyPos) {
        state.companyApplyPos = companyApplyPos;
    },
...
const actions = {
    ...
    //后台查看个人公司职位
    async showCompanyPos({commit},data){
    ...
    //后台查看个人公司申请
    async companyApplyPos({commit}){
        let result = await reqShowCompanyApplyPos();
        console.log(result);
        if(result.data.code == 1){
            commit("COMPANYAPPLYPOS",result.data.name)
            return 'ok';
        }else{
         return Promise.reject(new Error("faile"));
        }
...
const getters = {
    ...
    companyApplyPos(state) {
        return state.companyApplyPos || {};
    },
    adminEdu(state) {
