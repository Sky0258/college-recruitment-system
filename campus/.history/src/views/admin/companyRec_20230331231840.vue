<template>
  <div class="box">
    <!-- 信息框 -->
    <div class="three">
      <div class="message" v-for="item in info" :key="item.id" v-show="info.size()">
        <div class="msgRight">
...
export default {
  ...
  mounted() {
    this.showCompanyPos();
    this.info =  this.companyPos.records;
  },
  methods: {
    showCompanyPos() {
      this.$store
        .dispatch("showCompanyPos", {
          page: 1,
          size: 3,
        })
        .then(() => {
          this.info = this.companyPos.records;
          this.totalPage = this.companyPos.total;
          this.currentPage1 = 1;
        })
        .catch(() => {
  ...
  computed: {
    ...mapGetters(["companyPos","selfInfo"]),
  },
};
</script>
